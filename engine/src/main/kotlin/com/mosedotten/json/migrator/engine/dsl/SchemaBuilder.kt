...
) = SchemaBuilder(rootNode, versionField, allowMissingVersionField, execution).apply(block).execute()

@JsonMigratorDsl
class SchemaBuilder internal constructor(
    private val rootNode: ObjectNode,
    private val versionField: String,
    private val allowMissingVersionField: Boolean,
    private val execution: ExecutionStrategy,
) {
    private val migrations = mutableListOf<Migration>()

    fun migration(from: Int, to: Int, block: MigrationBuilder.() -> Unit) {
        migrations += MigrationBuilder(from, to).apply(block).build(versionField, allowMissingVersionField, execution)
    }
