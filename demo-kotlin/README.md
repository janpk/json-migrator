# json-migrator-demo-kotlin

A worked example of the migration library's core use case, built with the Kotlin `schema { }` DSL:
...
- The application holds **one DTO** — [`CreditApplication`](src/main/kotlin/com/mosedotten/json/migrator/demo/CreditApplication.kt),
...
- On read, a document is fed through the migration script, which upgrades it from whatever version it
  declares to the latest, and is then deserialized into the DTO for processing. For the database case
  it is serialized straight back out — so accessing an old row rewrites it at the current version.
- Migration is just-in-time per document; there is no bulk upgrade of the whole database.

## The migration script

[`CreditApplicationMigrations`](src/main/kotlin/com/mosedotten/json/migrator/demo/CreditApplicationMigrations.kt)
is a single `schema { }` pipeline. Over the application's lifetime it grew one step per release
(the first release shipped only `1 → 2`, the next added `2 → 3`, and so on); what you see is the
script as it stands now that the application is at v6. The engine reads a document's current version,
skips the steps it has already applied, and runs the rest through to the latest.
...
- [`JustInTimeMigrationDemo`](src/test/kotlin/com/mosedotten/json/migrator/demo/JustInTimeMigrationDemo.kt) —
  reads a document stored at each version (v1..v6), migrates it to the latest, and deserializes it into
  the one application DTO.
- [`DatabaseRoundTripDemo`](src/test/kotlin/com/mosedotten/json/migrator/demo/DatabaseRoundTripDemo.kt) —
...
```bash
./mvnw -pl demo-kotlin -am test
```

See [json-migrator-demo-java](../demo-java) for the same scenario built with the Java `JsonMigrator` facade.
